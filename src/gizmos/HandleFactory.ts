...
export const geo = {
  ...
  ringPicker: (t: GizmoTheme, radius: number) =>
    // keep pick tube ~0.078 even when the visual tube changes
    new TorusGeometry(radius, Math.max(t.sizes.ringTube * t.sizes.pickerScale, 0.078), 8, 32),
  cube: (t: GizmoTheme) => new BoxGeometry(t.sizes.gripSize, t.sizes.gripSize, t.sizes.gripSize),
  octa: (t: GizmoTheme) => new OctahedronGeometry(t.sizes.gripSize * 0.8, 0),
